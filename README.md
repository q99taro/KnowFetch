...
4. **梳理為知識關聯圖**：AI 再次上場，把這些小段落閱讀過後，提煉出「術語」、「概念」或「程式碼範例」。系統接著會把它們之間的關聯性（例如：`read_csv` 是屬於 `Pandas` 套件的方法）存進資料庫，形成巨大的知識網。
5. **到期推播複習**：每天晚上，系統的「間隔重複演算法 (FSRS)」會幫你計算出今天大腦該複習哪個知識點了。時間一到，你的 **Telegram** 就會收到一張精美的卡片，上面有概念說明以及搭配的 Python 程式碼上下文，讓你用一分鐘就能在通勤時輕鬆鞏固記憶！
